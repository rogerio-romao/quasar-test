  ...
  <section id="featured" ref="sectionRef" class="py-24 bg-[#0a0a0f]">
    <div class="max-w-7xl mx-auto px-6">
      ...
      <div class="mb-16 reveal">
        <p class="font-mono text-[#6366f1] text-sm tracking-[0.2em] uppercase mb-3">Selected Work</p>
        <h2 class="font-syne font-bold text-4xl md:text-5xl text-[#e4e4e7]">Featured Projects</h2>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <div class="relative aspect-video overflow-hidden">
            ...
            <!-- Hover overlay -->
            <div class="absolute inset-0 bg-[#0a0a0f]/85 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
              <div class="translate-y-3 group-hover:translate-y-0 transition-transform duration-300">
                <h3 class="font-syne font-bold text-lg text-[#e4e4e7] mb-2">{{ project.name }}</h3>
                <p class="font-manrope text-[#a1a1aa] text-sm mb-4 line-clamp-2">{{ project.shortDesc }}</p>
                <span class="font-mono text-xs text-[#f0b429] border border-[#f0b429]/60 px-3 py-1">
                  View Details →
                </span>
              </div>
            </div>
            ...
            <div class="absolute inset-0 border-2 border-transparent group-hover:border-[#f0b429]/30 transition-all duration-300 pointer-events-none"></div>
          </div>

          <!-- Card footer -->
          <div class="p-4 border-t border-[#1a1a2e]">
            <h3 class="font-syne font-semibold text-[#e4e4e7] text-sm truncate mb-2">{{ project.name }}</h3>
...
export default defineComponent({
  ...
  setup () {
    ...
    const getTechs = (featuredItem) => {
      ...
      const match = allProjects.value.find(p => p.id === featuredItem.projectRef)
